...
window.onload = function() {
    ...
    var operateNum = function(num1, num2, op) {
        ...
        //显示面板 
    var calculate_outPut = document.getElementById("srceen");
    //操作面板 
    var calculate_num = document.getElementById("keyboard");
    var result = 0; //计算结果 
    ...
    function setScreen(num) {
        ...
        setInnerText(calculate_outPut, num);
    }
    //获取显示屏的值 
    function getScreen() {
        return getInnerText(calculate_outPut);
    }


    //添加点击事件 
    var calculateNumHandler = function(e) {
        ...
        if (target.type == "button") {
            ...
            if (mark === "ops") { //如果点击的是操作符则设计第一个操作数 
    ...
    //添加事件监听器
    EventUtil.addHandler(calculate_num, "click", calculateNumHandler);
}


function getInnerText(element) {
    return (typeof element.textContent == "string") ? element.textContent.trim() : element.innerText.trim();
}
